...
| 終値・前日比・52週高安・出来高 | Yahoo Finance（時価総額と同じバッチで取得し `prices.json` に保存。マイ銘柄タブの終値表示とアラート判定で使用） |
| マイ銘柄の終値アラート | 平日引け後、[`scripts/generate_alerts.py`](scripts/generate_alerts.py) がマイ銘柄の終値をチェックして `alerts.json` を生成（ホーム画面に表示）。メール通知ONの場合はGitHub Issueを作成し@メンション → GitHubの通知メールが届く |
...
8. **⚙ 設定** 画面で、重要度ごとの終値アラート条件（株価変動±X%・52週高値/安値・
   出来高急増・決算前日/当日）とメール通知、gitへの自動バックアップを設定できます。
   アラートはホーム画面に表示されます。
