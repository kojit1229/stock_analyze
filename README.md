...
11. **市況** 画面で当日の市場全体を確認できます — 自動生成の概況コメント、
    値上がり/値下がり銘柄数、平均騰落率（単純/時価総額加重）、騰落レシオ
    （終値履歴から最大25日で算出）、売買代金合計・時価総額合計、上昇/下落業種数、
    ネット新高値、本日の開示件数、セクター別騰落（33業種）、売買代金ランキング、
    値上がり/値下がり率ランキング、52週高値/安値更新（年初来・上場来の代替）、
    指数インパクト（時価総額増減額）、出来高急増、市場区分別騰落、今週の決算発表件数。
    毎営業日の引け後に**スナップショットがgitに保存**され、日付セレクタで過去の市況を
    表示したり、主要指標の**推移チャート**で地合いの傾向を確認できます。
    「📝 MDレポート」から**Markdown形式のレポート**を開けるので、そのまま生成AIに
    渡して「直近の傾向は？」のような質問ができます（`frontend/data/market/*.md`）。
