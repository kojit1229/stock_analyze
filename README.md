...
| 決算短信・訂正短信・業績予想修正・決算説明資料 | TDnet 適時開示（やのしんWebAPI経由）。PDFはTDnetの実URLを開きます |
...
マイ銘柄・閲覧済みフラグ・コメントはブラウザの localStorage に保存されます（マイ銘柄画面からJSONファイルへバックアップ/復元できます）。`data/*.json` が無い環境では従来どおりサンプルデータで動作します（「デモ」バッジ）。
