...
マイ銘柄・閲覧済みフラグ・コメント・アラート設定などの個人状態は、ブラウザの localStorage にのみ保存されます。端末変更やキャッシュクリアに備える場合は、マイ銘柄画面から手動でJSONファイルをバックアップ/復元できます。**⚙ 設定**画面のGitHubトークン（このリポジトリのContents権限のみのFine-grained PAT）は、決算短信PDFの恒久保存リスト（`config/pdf_watchlist.json`）を編集する場合にのみ使用します。終値アラートは、入力だった `config/user_data.json` の自動コミット経路の廃止に伴い**現在は生成されません**。`data/*.json` が無い環境では従来どおりサンプルデータで動作します（「デモ」バッジ）。
...
8. **⚙ 設定** 画面で、重要度ごとの終値アラート条件（株価変動±X%・52週高値/安値・
   ...
   PDF保存リスト編集用のGitHubトークンを設定できます。ただし終値アラートは現在
   停止中です（入力データの自動コミット経路を廃止したため。設定値は保存されますが
   使われません）。
